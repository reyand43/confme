...
import { FETCH_AGENDA_EVENTS_ERROR, FETCH_AGENDA_EVENTS_START, FETCH_AGENDA_EVENTS_SUCCESS} from "./actionTypes";

export function fetchAgendaEvents(userId) {
    return async (dispatch) => {
      dispatch(fetchAgendaEventsStart());
      try {
        db.ref("/users/"+userId+"/personalEvents").on("value", function (snapshot) {
          let agendaEvents = []
